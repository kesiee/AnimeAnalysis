...
import com.utilities.Utilities.write_parquet
...
object DataCleaning {
  ...
  def main(args:Array[String]):Unit= {
    ...
    write_parquet(anime_dataset_df, clean_write_path + "anime_dataset")
    write_parquet(user_details_df, clean_write_path + "user_details")
    ...
    for (file <- files) {
      ...
      write_parquet(user_score_df, user_score_write_path + "user_score_" + file.getName.stripSuffix(".csv"))
    }
